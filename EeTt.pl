#!/usr/bin/env perl
# Authors: Zheng Sun, Rongchao Zhang, Shi Huang 2020.06.03
use strict;
use warnings;
use Getopt::Long;
use FindBin qw($Bin);
...
Required:
    ...
    -t  <int>    The type of input fasta/fastq file
                 ...
                 [3] SE sequencing data in the Fastq format
    ...
    -op <str>    The output prefix (default: the sample name)
Optional:
    -gz <str>    Whether the output file is compressed [$gz] (yes or no)
Optional (only applicable when -t equals 2, or 3, or 4, i.e. taking fastq data input):
    -qc <str>    Whether quality control is required [$q_control] (yes or no)
    -n  <float>  Maximum percentage of ambiguity bases \"N\" [$ncount]
    -q  <int>    Minimum per-base quality score (XX) [$quality]
    -p  <int>    Minimum percentage of bases that must have per-base quality score over [-q] [$percent]
    -b  <int>    Quality values base [$qbase]
    -fm <str>    Output file format [$format] (fa or fq)
...
sub flash{
	...
	&execute("$flash -t 1 -z -q -o $outdir/$outprefix $r1 $r2");#flash 拼接，当插入片段过短时，需要调整flash参数，默认参数会偏低
...
sub Five_Lable{# Data split for five concanated 2b-RAD tags
	...
	my $r2=$input[1];
	my ($output,@fhandle);
	my $outprefix=$outprefix[0];# 使用第一个样品进行中间文件命名
	my $input_reads_num;
...
sub Single_Lable{
	...
	while(<IN>){
		...
		my $line=$_ . <IN> . <IN> . <IN>;
		my @tmp=split /\n/,$line;
		if(length($tmp[1])>50){# if input read length exceeds 50bp (such as those from PE150 platform), we will chop the first 50-bp for the following analysis 如果reads长度大于50bp则进行截取，比如在PE150测序的单标签2b数据
