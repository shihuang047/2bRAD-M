...
Description
    ...
    3.To extract single 2b-RAD tags from SE sequencing data, run: perl EeTt.pl -i 2b-RADsingle.fq(.gz or 2b-RADsingle.1.fq.gz) -t 3 -s 1 -od . -op sample
    4.To split five concatenated 2b-RAD tags from PE sequencing data, run: perl EeTt.pl -i R1.fq(.gz) R2.fq(.gz) -t 4 -s 1 -od . -op sample1 sample2 sample3 sample4 sample5
Usage
...
Optional (only applicable when -t equals 2, or 3, or 4, i.e. taking fastq data as input):
    ...
    -q  <int>    Minimum per-base quality score (default: 25) [$quality]
    -p  <int>    Minimum percentage of bases that must have per-base quality score over [-q] (default: 80) [$percent]
    -b  <int>    Phred quality score type (default: 33)  [$qbase]
