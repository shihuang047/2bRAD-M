...
my $remove_redundant ||="yes";# 基因组内部是否去冗余 yes or no, default value is "no"
...
unless(defined($enzyme_file)){#未提供酶切文件则进行酶切
	print STDOUT "###($enzyme) Electron digestion -- start, ",`date`;# output the log file
	my $complete=0;
	my %complete;
	$enzyme_file="$outdir/$enzyme.enzyme.fa.gz";
	open OU,"|gzip > $outdir/$enzyme.enzyme.fa.gz" or die "cannot open $outdir/$enzyme.enzyme.fa.gz\n";
	if($list=~/\.gz/){
		open LI,"gzip -dc $list|" or die "cannot open $list\n";
	}else{
		open LI,"$list" or die "cannot open $list\n";
	...
	while(<LI>){
		...
		$/=">";
		if($tmp[-1]=~/\.gz$/){
			open IN,"gzip -dc $tmp[-1]|" or die "cannot open $tmp[-1]\n";
		}else{
			open IN,"$tmp[-1]" or die "cannot open $tmp[-1]\n";
		}
		<IN>;
		my $ingenome_tag_num=0;
		while(<IN>){
			chomp;
			my @a=split /\n/;
			my $id=(split /\s+/,$a[0])[0];#scaffold id
			my $seq=join("",@a[1..$#a]);#scaffold seq
			...
			for my $i(0..$#site){# iterate all restriction sites 循环酶切位点
				while($seq=~/($site[$i])/g){# digital digestion
					my $tag=$1;#序列
					my $len=length($tag);#标签长度
					my $pos=pos($seq);
					my $start=$pos-$len+1;#标签起始位置
					pos($seq)=$start;#调整位置
					#GCF号|基因组内部标签排序|scaffoldid|startpos|正反向酶切|是否为指定水平下unique&&noredundancy标签
					$hash_genome{$start}=">$tmp[0]|0|$id|$start|0|-\n$tag\n";
				}
			}
			# sort the 2b-RAD tags by the genome positions排序后输出
			for my $pos(sort {$a <=> $b} keys %hash_genome){
				$ingenome_tag_num++;
				my @a=split /\|/,$hash_genome{$pos};
				print OU "$a[0]|$ingenome_tag_num|",join("|",@a[2..$#a]);
			}
			undef %hash_genome;
		}
		close IN;
	...
	print STDOUT "An enzyme digestion file has been provided, skipping electron digestion.\n";
}
...
for my $level(sort {$hs_type{$a}<=>$hs_type{$b}} keys %hs_type){ #iterate all taxonomic levels of 2b-RAD database
	...
	while(<IN>){
		...
		$hash_gcf_rank{$gcfid}++;
		my @a=split /\t/,$hash_gcf2class{$gcfid};
	...
	while(<IN>){
		...
		$gcfid=~s/^>//;
		next unless(exists $hash_gcf2class{$gcfid});#gcfid不在列表中则跳过
		$hash_genome_tag_num{$gcfid}++;#基因组电子酶切标签数
		...
		for (my $i=100;$i>0;$i=$i-1){ #每完成1%则输出进度
			if((keys %hash_genome_tag_num)/$genome_total_num*100>=$i){
				print STDOUT "$i% " unless(exists $complete{$i});#仅没输出过的才会输出日志
				$complete{$i}++;
				last;
			}
	...
	while(<LI>){
		...
		my @tmp=split /\t/,$line;
		print STAT "$tmp[0]";
		if(exists $hash_genome_tag_num{$tmp[0]}){#酶切标签数
			print STAT "\t$hash_genome_tag_num{$tmp[0]}";
		}else{
			print STAT "\t0";
		}
